		...
		doManager("storeResourcesManager","findStoreResourcesByType",null,
			...
			if(data.result){
				...
				if(typeof(jsonData.storeResource) != 'undefined'){
                    ...
                    var storecount = 0;
                    var home_screen = 0;
                    var home_auxiliary_screen = 0;
                    var outdoor_electronic_screen = 0;
                    var roll_up = 0;
                    var posters = 0;
                    var central_pile_head = 0;
                    var floor_pile_head = 0;
                    var indoor_interactive_area = 0;

                    for(var i = 0; i < storeResource.length; i++){
						var info = storeResource[i];
						storecount += info.storecount;
                        home_screen += info.home_screen;
                        home_auxiliary_screen += info.home_auxiliary_screen;
                        outdoor_electronic_screen += info.outdoor_electronic_screen;
                        roll_up += info.roll_up;
                        posters += info.posters;
                        central_pile_head += info.central_pile_head;
                        floor_pile_head += info.floor_pile_head
                        indoor_interactive_area += info.indoor_interactive_area;
						...
						if(i == storeResource.length -1){
                            storeStr += '<tr><th>'+(i+2)+'</th><th>合计</th><td>'+storecount+'</td><td>'+home_screen+'</td><td>'+home_auxiliary_screen+'</td>'
                                +'<td>'+outdoor_electronic_screen+'</td><td>'+roll_up+'</td><td>'+posters+'</td><td>'+central_pile_head+'</td>'
                                +'<td>'+floor_pile_head+'</td><td>'+indoor_interactive_area+'</td></tr>';
                        }
					}
				}else{
				...
				if(typeof(jsonData.outStoreResource) != 'undefined'){
                    ...
                    var storecount = 0;
                    var frame_number = 0;
                    var associated_community_count = 0;
                    var open_community_count = 0;
                    var closed_community_count = 0;
                    var activity_area_count = 0;
                    var charge_for_site_count = 0;
                    var free_for_site_count = 0;
                    var site_area = 0;
                    var associated_community_town_count = 0;
					for(var i = 0; i < outStoreResource.length; i++){
						var info = outStoreResource[i];
                        storecount += info.storecount;
                        frame_number += info.frame_number;
                        associated_community_count += info.associated_community_count;
                        open_community_count += info.open_community_count;
                        closed_community_count += info.closed_community_count;
                        activity_area_count += info.activity_area_count;
                        charge_for_site_count += info.charge_for_site_count;
                        free_for_site_count += info.free_for_site_count;
                        site_area += info.site_area;
                        associated_community_town_count += info.associated_community_town_count;
						...
						if(i == outStoreResource.length -1){
                            outStoreStr += '<tr><th>'+(i+2)+'</th><th>合计</th><td>'+storecount+'</td><td>'+frame_number+'</td><td>'+associated_community_count+'</td><td>'+open_community_count+'</td>'
                                +'<td>'+closed_community_count+'</td><td>'+activity_area_count+'</td><td>'+charge_for_site_count+'</td><td>'+free_for_site_count+'</td><td>'+site_area+'</td>'
                                +'<td>'+associated_community_town_count+'</td></tr>';
                        }
					}
				}else{
