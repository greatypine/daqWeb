...
public class HumanresourcesDaoImpl extends DAORootHibernate implements HumanresourcesDao{
	...
	//取得店长最大员工编号
		...
		public List<Map<String, Object>> queryHumanTotal(String zw) {
			...
			String sql = "";
			String sql_humanresources = "select sum(CASE when humanstatus != 2 and date_format(topostdate,'%Y-%m-%d') <= date_format(date_sub(NOW(),INTERVAL date_format(NOW(), '%w') -"+settime+"+7*5 DAY),'%Y-%m-%d') then 1 else 0 end) as a,"
			...
			if("店长".equals(zw)){
				...
				sql = "select sum(CASE when humanstatus != 2 and date_format(topostdate,'%Y-%m-%d') <= date_format(date_sub(NOW(),INTERVAL date_format(NOW(), '%w') -"+settime+"+7*5 DAY),'%Y-%m-%d') then 1 else 0 end) as a,"
						...
						+"sum(CASE when humanstatus != 2 and date_format(topostdate,'%Y-%m-%d') <= date_format(date_sub(NOW(),INTERVAL date_format(NOW(), '%w') -"+settime+"+7*2 DAY),'%Y-%m-%d') then 1 else 0 end) as d,"
						+"sum(CASE when humanstatus != 2 and date_format(topostdate,'%Y-%m-%d') <= date_format(date_sub(NOW(),INTERVAL date_format(NOW(), '%w') -"+settime+"+7 DAY),'%Y-%m-%d') then 1 else 0 end) as e,"
						+"sum(CASE when humanstatus != 2 and date_format(topostdate,'%Y-%m-%d') <= date_format(date_sub(NOW(),INTERVAL date_format(NOW(), '%w') -"+settime+" DAY),'%Y-%m-%d') then 1 else 0 end) as f from t_humanresources where zw = '国安侠'";
			}else if("专员".equals(zw)){
				sql = "select sum(CASE when humanstatus != 2 and date_format(topostdate,'%Y-%m-%d') <= date_format(date_sub(NOW(),INTERVAL date_format(NOW(), '%w') -"+settime+"+7*5 DAY),'%Y-%m-%d') then 1 else 0 end) as a,"
						+"sum(CASE when humanstatus != 2 and date_format(topostdate,'%Y-%m-%d') <= date_format(date_sub(NOW(),INTERVAL date_format(NOW(), '%w') -"+settime+"+7*4 DAY),'%Y-%m-%d') then 1 else 0 end) as b,"
						+"sum(CASE when humanstatus != 2 and date_format(topostdate,'%Y-%m-%d') <= date_format(date_sub(NOW(),INTERVAL date_format(NOW(), '%w') -"+settime+"+7*3 DAY),'%Y-%m-%d') then 1 else 0 end) as c,"
		...
		public List<Map<String, Object>> querySixWeekHuman() {
			...
			String sql = "";
				String sql_a = "select count(employee_no) as leavecount, citySelect as cityname,'' as topostcount from " +
						"(select employee_no,citySelect from t_humanresources where humanstatus = 2 and date_format(leavedate,'%Y-%m-%d') > date_sub(NOW(),INTERVAL date_format(NOW(), '%w') -"+settime+"+7*6 DAY) and date_format(leavedate,'%Y-%m-%d') <= date_sub(NOW(),INTERVAL date_format(NOW(), '%w') -"+settime+" DAY) UNION " +
						"select employee_no,citySelect from t_storekeeper where humanstatus = 2 and date_format(leavedate,'%Y-%m-%d') > date_sub(NOW(),INTERVAL date_format(NOW(), '%w') -"+settime+"+7*6 DAY) and date_format(leavedate,'%Y-%m-%d') <= date_sub(NOW(),INTERVAL date_format(NOW(), '%w') -"+settime+" DAY) and zw = '店长') " +
						"t GROUP BY t.citySelect";
				String sql_b="select count(employee_no) as topostcount, citySelect as cityname,'' as leavecount from " +
						"(select employee_no,citySelect from t_storekeeper where date_format(topostdate,'%Y-%m-%d') > date_sub(NOW(),INTERVAL date_format(NOW(), '%w') -"+settime+"+7*6 DAY) and date_format(topostdate,'%Y-%m-%d') <= date_sub(NOW(),INTERVAL date_format(NOW(), '%w') -"+settime+" DAY) and zw = '店长' UNION " +
						"select employee_no,citySelect from t_humanresources where date_format(topostdate,'%Y-%m-%d') > date_sub(NOW(),INTERVAL date_format(NOW(), '%w') -"+settime+"+7*6 DAY) and date_format(topostdate,'%Y-%m-%d') <= date_sub(NOW(),INTERVAL date_format(NOW(), '%w') -"+settime+" DAY)) " +
						"t GROUP BY t.citySelect";
