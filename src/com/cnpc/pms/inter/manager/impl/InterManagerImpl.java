...
//	}
	...
	//接口 查询城市列表
	@Override
	public Result queryCityNameList(){
		Result result = new Result();
		try {
			DistCityCodeManager distCityCodeManager = (DistCityCodeManager) SpringHelper.getBean("distCityCodeManager");
			List<DistCityCode> distCityCodes = (List<DistCityCode>) distCityCodeManager.getList();
			if(distCityCodes!=null&&distCityCodes.size()>0){
				List<CityCodeDto> cityCodeDtos = new ArrayList<CityCodeDto>();
				for(DistCityCode dcc:distCityCodes){
					CityCodeDto ccd = new CityCodeDto();
					ccd.setCity_name(dcc.getCityname());
					cityCodeDtos.add(ccd);
				}
				result.setCode(CodeEnum.success.getValue());
		        result.setMessage(CodeEnum.success.getDescription());
		        result.setData(cityCodeDtos);
			}else{
				result.setCode(CodeEnum.nullData.getValue());
		        result.setMessage(CodeEnum.nullData.getDescription());
			}
						...
						if(skid!=null) {
							...
							if(user!=null&&user.getMobilephone()!=null) {
								...
								if(user.getMobilephone()!=null&&user.getMobilephone().trim().length()>0) {
									rt = commonSendMessage(user.getMobilephone(), "您有一条选品建议（库存预警）消息，请点击 "+sendShortUrl.getShorturl()+" 查看 ", "");
								}else {
									rt="电话为空，未发送";
								}
