...
public class CostStatisticsDaoImpl extends BaseDAOHibernate implements CostStatisticsDao {
    ...
    public List<Map<String, Object>> queryCostLabor(CostDto costDto) {
        ...
        if ("zb".equals(costDto.getRole())) {// 总部
            ...
            storeSql = "select t.name,t.store_id,t.platformid,t.number,t.storeno,t.city_name,t1.citycode,t.estate,t.storetype from t_store t  inner join  (select * from t_dist_citycode "
                    + cityStr + ") t1" + " on t.city_name  = t1.cityname  where t.name not like '%储备%' and t.name not like '%办公室%' and t.flag='0' and t.storetype!='V' and t.storetype!='W'  and t.name not like '%测试%' "+ storeStr ;
            ...
            if (costDto.getUserId() != null && !"".equals(costDto.getUserId())) {
                storeSql = "select t.name,t.store_id,t.platformid,t.number,t.storeno,t.city_name,t1.citycode,t.estate,t.storetype from t_store t  inner join  (select tdc.id,tdc.cityname,tdc.citycode from t_dist_citycode tdc "
                        ...
                        + cityStr + ") t1"
                        + "	on t.city_name  = t1.cityname  where t.name not like '%储备%' and t.name not like '%办公室%' and t.flag='0' and t.storetype!='V' and t.storetype!='W'  and t.name not like '%测试%' " + storeStr;
        ...
        if(costDto.getEstate()!=null&&!"".equals(costDto.getEstate())){
            sql+=" and ts.estate ='"+costDto.getEstate()+"'";
        }
    ...
    public List<Map<String, Object>> exportCostLabor(CostDto costDto) {
        ...
        if(costDto.getEstate()!=null&&!"".equals(costDto.getEstate())){
            sql+=" and a.estate ='"+costDto.getEstate()+"'";
        }
    ...
    public List<Map<String, Object>> queryCostRent(CostDto costDto) {
        ...
        if ("zb".equals(costDto.getRole())) {// 总部
            ...
            if(costDto.getEstate()!=null&&!"".equals(costDto.getEstate())){
                storeStr += " and t.estate='"+costDto.getEstate()+"'";
            }
        ...
        String sql = " select ts.storeno as store_no,ts.name as store_name,ts.city_name,ts.estate,ts.address as addr,tcr.property_fee_year,tcr.property_fee_month,tcr.property_deadline,tcr.rental_month,tcr.cost_month from ("+storeSql+") ts " +
...
//                    " (select IFNULL(contract_grand_total,0)/60 as rentalMonth_cal,storeNo as contractStoreNo from t_cost_rent_contract where lease_stop_date >= CONCAT('"+costDto.getrDate()+"','01')) tcrc"+
                    "  where ts.name not like '%储备%' and ts.name not like '%办公室%'  and ts.storetype!='V' and ts.storetype!='W'  and ts.name not like '%测试%'";
    ...
    public List<Map<String, Object>> exportCostRent(CostDto costDto) {
        ...
        String sql = " select ts.storeno as store_no,ts.city_name,ts.name as store_name,ts.city_name,ts.address as addr,ts.estate,tcr.*,tcrc.* from ("+storeSql+") ts " +
                " left join ("+sqlSub+") tcr on ts.storeno = tcr.storeNo"+
                " LEFT JOIN"+
                ...
                " on ts.storeno = tcrc.storeNo where  ts.name not like '%储备%' and ts.name not like '%办公室%' and ts.name not like '%测试%'  and ts.storetype!='V' and ts.storetype!='W'";
    ...
    public List<Map<String, Object>> queryCostRenovation(CostDto costDto) {
        ...
        String sql ="select ts.estate,ts.storeno as store_no,ts.name as store_name ,ts.city_name,tcr.decoration_company,tcr.structure_acreage,tcr.renovation_unit_price,tcr.decorate_cost,tcr.business_screen,tcr.furniture,tcr.light_box,tcr.process_manage,tcr.process_manage_surcharge,tcr.air_conditioner,tcr.air_conditioner_surcharge,tcr.whole_process_manage_surcharge,tcr.design,tcr.total,tcr.amortize_month,tcr.amortize_money,tcr.completed_date,tcr.contract_date from ("+storeSql+") ts left join ("+sqlSub+") tcr on  ts.storeno = tcr.storeNo  where  ts.name not like '%储备%' and ts.name not like '%办公室%' and ts.name not like '%测试%'  and ts.storetype!='V'";
    ...
    public List<Map<String, Object>> queryCostFixedAsset(CostDto costDto) {
        ...
        String sql ="select ts.estate,ts.storeno as store_no,ts.name as store_name ,ts.city_name,tcfa.amortize_money,tcfa.total,tcfa.aio,tcfa.mobile_phone,tcfa.iPad,tcfa.cash_register,tcfa.computer,tcfa.scanner_gun,tcfa.electronics_total,tcfa.electronics_amortize,tcfa.electric_cars,tcfa.electric_cars_amortize,tcfa.cold_chain,tcfa.safe_box,tcfa.capsule_goods_shelf,tcfa.shopping_goods_shelf,tcfa.machine_total,tcfa.machine_amortize from  ("+storeSql+") ts left join ("+sqlSub+") tcfa on  ts.storeno = tcfa.storeNo  where  ts.name not like '%测试%'  and ts.storetype!='V'";
    ...
    public List<Map<String, Object>> queryCostGWE(CostDto costDto) {
        ...
        String sql="SELECT ts.storeno as storeNo,ts.city_name,ts.name as storeName ,ts.storeType,ts.estate,tc.* " +
                " FROM ("+storeSql+") ts left join  (SELECT storeNo,gas_fee,water_fee,electricity_fee FROM t_cost_gas_water_elec where year="+costDto.getYear()+" and month="+costDto.getMonth()+") tc on ts.storeno=tc.storeNo  where  ts.name not like '%测试%'  and ts.storetype!='V'";
    ...
    public List<Map<String, Object>> exportCostGWE(CostDto costDto) {
        ...
        String sql="SELECT ts.storeno ,ts.name as storeName ,ts.city_name,ts.storeType,ts.estate,tc.* " +
                ...
                "  FROM `t_cost_gas_water_elec` where year="+costDto.getYear()+" group by storeNo) tc on ts.storeno=tc.storeNo  where  ts.name not like '%测试%'  and ts.storetype!='V'";
    ...
    public List<Map<String, Object>> queryCostOperation(CostDto costDto) {
        ...
        String sql="SELECT ts.storeno as store_no,ts.city_name,ts.name as store_name ,ts.storeType,ts.estate,tc.* " +
                " FROM ("+storeSql+") ts left join  (select * from t_cost_operation where year="+costDto.getYear()+" and month="+costDto.getMonth()+") tc on ts.storeno=tc.storeNo  where  ts.name not like '%测试%'  and ts.storetype!='V'";
    ...
    public List<Map<String, Object>> queryCostStatistic(String cityName, String storeNo, Integer year, Integer month,PageInfo pageInfo) {
        ...
        String sql = "select ts.city_name as cityName,tdc.id as cityId,ts.total,ts1.total as opentotal,l.labor,r.rent,rv.renovation,u.uniform,rc.rentContract,fa.fixedAsset,g.gwe,co.operation from " +
                " (select city_name,count(store_id) as total from t_store where name not like '%储备%' and name not like '%办公室%' and flag='0' and storetype!='V' and storetype!='W'  and name not like '%测试%'  "+sqlSub+" GROUP BY city_name) ts" +
                " left join "+
                " (select city_name,count(store_id) as total from t_store where name not like '%储备%' and name not like '%办公室%' and flag='0' and storetype!='V' and storetype!='W' and ifnull(estate,'') not like '%闭店%' and name not like '%测试%'  "+sqlSub+" GROUP BY city_name) ts1 on ts.city_name =ts1.city_name " +
                " left join " +
