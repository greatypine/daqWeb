...
//			}
	...
	public Map<String, Object> queryMemberInvitation(DynamicDto dynamicDto, PageInfo pageInfo) {
		...
		String sql=" select IFNULL(b.inviteCode,'暂无') as inviteCode,ifnull(a.total,0) as total,b.employee_no,CONCAT('*******',SUBSTR(b.mobilephone,8,11)) as mobilephone,b.name,GROUP_CONCAT(b.storename) as storename,b.city_name from "
				...
				+" RIGHT JOIN"
				+" (select t.name,t.phone as mobilephone,t.employee_no,t.inviteCode,ifnull(ts.name,'') as storename,ifnull(t.citySelect,'') as city_name from t_humanresources t LEFT JOIN t_store ts ON t.store_id = ts.store_id where t.humanstatus=1  and t.inviteCode is not null and t.inviteCode!='' "+whereStoreId
...
/*		String sql = "SELECT IFNULL(FLOOR(sum(dom.trading_price)), 0) AS week_gmv,date_format(dom.sign_time, '%m-%d') AS week_date FROM df_mass_order_monthly dom  " +
	...
	public Map<String, Object> getCustomerMember(DynamicDto dynamicDto, PageInfo pageInfo) {
		...
		String sql = "select t1.opencount,ifnull(t2.nowcount,0) as nowcount from (select count(member.customer_id) as opencount from df_user_member member where  member.associator_expiry_date>now() and  member.opencard_time is null) t1,(" +
	...
	public Map<String, Object> getUserBehaviorByLog(DynamicDto dynamicDto,String cityNo, PageInfo pageInfo) {
		...
		if(dynamicDto.getSearchstr().equals("user_active_store")){
			...
			sql = "select tsa.name as city_name,sum (isnull( T_2.visit_num , 0)) as visit_num,sum (isnull( T_3.add_num , 0) ) as add_num,sum (isnull( T_4.order_num , 0) ) as order_num," +
					"sum (isnull( T_5.sign_num , 0)) as sign_num from " +
					"(select fnv_hash(concat(T_1.city_code ,T_1.store_id)) as id,T_1.city_code ,T_1.store_id ,count(1) as visit_num  from ( select max(lf.city_code) as city_code," +
					"max(lf.store_id) as store_id from datacube_kudu.log_final lf where 1=1 and lf.simple_date >= '"+dynamicDto.getBeginDate()+"' and lf.simple_date < '"+dynamicDto.getEndDate()+"' " +
					"and lf.customer_id is not null and lf.city_code is not null and lf.store_id is not null  group by lf.customer_id ) T_1 group by T_1.city_code ,T_1.store_id " +
					") T_2 left join " +
					"(select fnv_hash(concat(T_1.city_code ,T_1.store_id)) as id,T_1.city_code ,T_1.store_id ,count(1) as add_num from " +
					"(select max(lf.city_code) as city_code,max(lf.store_id) as store_id from datacube_kudu.log_final lf where 1=1 and lf.simple_date >= '"+dynamicDto.getBeginDate()+"' " +
					" and lf.simple_date < '"+dynamicDto.getEndDate()+"' and lf.customer_id is not null and lf.city_code is not null and lf.store_id is not null and lf.behavior_name in ( " +
					"'填写订单','获取用户选中购物车分组信息','预约服务时间','获取发货单详情','购物车全选','新立即购买','新团购下单','再来一单','积分商品下单页','可拆分商品预约(非团购)'," +
					"'填写订单','购物车列表','下单','选择全部','添加购物车','提交订单','改变分组状态','购物车删除','合并支付下单页','改变用户选中购物车分组状态')  group by lf.customer_id " +
					") T_1 group by T_1.city_code ,T_1.store_id " +
					") T_3 on T_2.id= T_3.id left join " +
					"(select fnv_hash(concat(T_1.city_code ,T_1.store_id)) as id, T_1.city_code , T_1.store_id, count(1) as order_num from ( " +
					"select max(ts.city_code) as city_code,max(ts.id) as store_id from  gemini.t_order tor  left join gemini.t_store ts on tor.store_id = ts.id " +
					"where 1=1 and tor.create_time > '"+dynamicDto.getBeginDate()+"' and tor.create_time < '"+dynamicDto.getEndDate()+"' and tor.store_id is not null group by tor.customer_id) T_1 " +
					"group by T_1.city_code , T_1.store_id " +
					")T_4 on T_3.id = T_4.id left join " +
					"(select fnv_hash(concat(T_1.city_code ,T_1.store_id)) as id, T_1.city_code, T_1.store_id, count(1) as sign_num from ( select max(ts.city_code) as city_code,max(ts.id) as store_id from " +
					"gemini.t_order tor left join gemini.t_store ts on tor.store_id = ts.id where 1=1  and tor.create_time > '"+dynamicDto.getBeginDate()+"' and tor.create_time < '"+dynamicDto.getEndDate()+"' and tor.store_id is not null  and tor.sign_time is not null group by tor.customer_id ) T_1 group by T_1.city_code,T_1.store_id " +
					") T_5 on T_4.id = T_5.id " +
					"left join gemini.t_sys_area tsa on T_2.city_code =  tsa.code " +
					"inner join gemini.t_store ts on T_2.store_id = ts.id  and ts.name not like '%测试%' " +appendSql+
					"group by tsa.name " +
