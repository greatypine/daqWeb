...
public class ChartStatDaoImpl extends BaseDAOHibernate implements ChartStatDao {
	...
	public Map<String, Object> queryMonthTurnover(ChartStatDto csd) {
		String sql = "SELECT IFNULL(FLOOR(sum(dst.order_amount)),0) as month_amount FROM ds_storetrade_channel dst WHERE dst.year = YEAR(curdate()) AND dst.month = MONTH(curdate()) "
	...
	public List<Map<String, Object>> queryTurnoverByWeek(ChartStatDto csd){
		String sql = "SELECT YEARWEEK(sign_time) as week_date ,	subdate(sign_time,date_format(sign_time,'%w')-7) as week_time, IFNULL(FLOOR(SUM(trading_price)),0) AS week_amount FROM df_mass_order_total WHERE store_name NOT LIKE '%测试%' ";
		if(StringUtils.isNotEmpty(csd.getStoreno())){
	...
	public List<Map<String, Object>> queryTurnoverByMonth(ChartStatDto csd){
		String sql = "SELECT IFNULL(FLOOR(SUM(dst.order_amount)),0) AS month_amount,CONCAT(dst.year,'-',dst.month) AS month_time,dst.month AS month_index from ds_storetrade_channel dst "
