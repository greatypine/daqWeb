...
//		List<Map<String, Object>> orderMonthCountList = orderDao.queryMonthOrderCount(dd,cityNO,provinceNO,"0");
				...
				if(sign!=null){
					...
					for(int k = 0; k < dynamicByCityList.size(); k++){
						if(dynamic.get("product_id").toString().equals(dynamicByCityList.get(k).get("product_id").toString())
								&&dynamic.get("cityno").toString().equals(dynamicByCityList.get(k).get("cityno").toString())){
							jo.put("cityrank", k+1);
						}
					}
				}
	...
	public Map<String, Object> getNewAreaData(String employee_no, String year, String month) {
	    ...
	    List<Map<String,Object>> list = interDao.queryCustomerMoreTenOfEmployee(employee_no, year, month);
	    if(list!=null&&list.size()>0){
	    	result.put("cusMoreTen",list.get(0).get("cusnum_ten"));
	    	
	    }else{
	    	result.put("cusMoreTen",0);
	    }
...
//				String cityCode = String.valueOf(orderCityMap.get("city_code"));
	...
	@Override
	public Map<String, Object> queryEmployeeSendOrders(DynamicDto dynamicDto,PageInfo pageInfo) {
		Map<String, Object> result = new HashedMap();
		StoreManager storeManager = (StoreManager)SpringHelper.getBean("storeManager");
		DynamicDao dynamicDao = (DynamicDao)SpringHelper.getBean(DynamicDao.class.getName());
		try {
	...
	public Map<String,Object> exportEmployeeSendOrders(DynamicDto dd){
		...
		Map<String,Object> result  = new HashMap<String,Object>();
		Map<String,Object> map  = this.queryEmployeeSendOrders(dd, null);
		if("success".equals(map.get("status"))){//成功返回数据
			List<Map<String, Object>> list = (List<Map<String, Object>>)map.get("sendorders");
			if(list==null||list.size()==0){
				result.put("message","没有符合条件的数据！");
				result.put("status","null");
				return result;
	...
	public List<Map<String, Object>> queryDepartment(){
		...
		List<Map<String, Object>> list = new ArrayList<Map<String, Object>>();
		try {
			list = dynamicDao.queryCityAllDept();
		} catch (Exception e) {
			e.printStackTrace();
	...
	@Override
	public Map<String, Object> queryMonthAndLastMonthTodayCustomerOrderCount(
			DynamicDto dd) {
		Map<String,Object> result = new HashMap<String,Object>();
		DynamicDao dynamicDao = (DynamicDao)SpringHelper.getBean(DynamicDao.class.getName());
		//查询上月当天的用户量
		List<Map<String, Object>> customerMonthAndLastMonthTodayCountList = dynamicDao.queryMonthAndLastMonthTodayCustomerOrderCount(dd);
		if(customerMonthAndLastMonthTodayCountList!=null&&customerMonthAndLastMonthTodayCountList.size()>0){
			result.put("last_customer_count", customerMonthAndLastMonthTodayCountList.get(0).get("customer_count"));
			result.put("last_order_count", customerMonthAndLastMonthTodayCountList.get(0).get("last_order_count"));
		}else{
			result.put("last_customer_count", 0);
			result.put("last_order_count", 0);
		}
		return result;		
	}
...
//		dd.setEndDate("2018-03-30");
	...
	@Override
	public Map<String, Object> getUserMemberInfo(DynamicDto dynamicDto,PageInfo pageInfo) {
		Map<String, Object> result = new HashMap<String,Object>();
		StoreManager storeManager = (StoreManager)SpringHelper.getBean("storeManager");
		DynamicDao dynamicDao = (DynamicDao)SpringHelper.getBean(DynamicDao.class.getName());
		try {
	...
	@Override
	public Map<String, Object> exportUserMemberInfo(DynamicDto dynamicDto) {
		Map<String,Object> result  = new HashMap<String,Object>();
		Map<String,Object> map  = this.getUserMemberInfo(dynamicDto, null);
		if("success".equals(map.get("status"))){//成功返回数据
	...
	@Override
	public Map<String, Object> selectMemberInvitation(DynamicDto dynamicDto, PageInfo pageInfo) {
		Map<String, Object> result = new HashedMap();
		StoreManager storeManager = (StoreManager)SpringHelper.getBean("storeManager");
		DynamicDao dynamicDao = (DynamicDao)SpringHelper.getBean(DynamicDao.class.getName());
		try {
...
//					}
					...
					dynamicDto.setStoreNo("-10000");
					dynamicDto.setStoreNumer("Y");//是否查询固定门店
				}else{

					Store  store = (Store)storeManager.getObject(dynamicDto.getStoreId());
					dynamicDto.setStoreNo(store.getStore_id().toString());
					dynamicDto.setStoreNumer("Y");
				}
			}else if(dynamicDto.getTarget()==1){//城市总监
				if(dynamicDto.getStoreId()==null||"".equals(dynamicDto.getStoreId())){//查询所有城市的门店
...
//					}
			...
			if("prev_month".equals(dynamicDto.getBeginDate())){
	            calendar1.set(Calendar.DAY_OF_MONTH, 1);
	            dynamicDto.setEndDate(sdf.format(calendar1.getTime()));
	            calendar1.add(Calendar.DAY_OF_MONTH,-1);
	            calendar1.set(Calendar.DAY_OF_MONTH, 1);
	            dynamicDto.setBeginDate(sdf.format(calendar1.getTime()));
			...
			result= dynamicDao.queryMemberInvitation(dynamicDto, pageInfo);
			result.put("status","success");
		} catch (Exception e) {
			e.printStackTrace();
			result.put("status","fail");
	...
	public Map<String, Object> exportMemberInvitation(DynamicDto dd) {
		Map<String,Object> result  = new HashMap<String,Object>();
		Map<String,Object> map  = this.selectMemberInvitation(dd, null);
		if("success".equals(map.get("status"))){//成功返回数据
			List<Map<String, Object>> list = (List<Map<String, Object>>)map.get("member");
			if(list==null||list.size()==0){
				result.put("message","没有符合条件的数据！");
	...
	public Map<String, Object> getsixWeekDate() {
		Map<String,Object> map = new HashMap<String,Object>();
		DynamicDao dynamicDao = (DynamicDao)SpringHelper.getBean(DynamicDao.class.getName());
		List<Map<String,Object>> getsixWeekDate = dynamicDao.getsixWeekDate();
		if(getsixWeekDate != null && getsixWeekDate.size() > 0){
			Map<String, Object> map2 = getsixWeekDate.get(0);
			map = map2;
		}
		return map;
	}
...
//    		String curDate = "2018-03-08";
			...
			dd.setBeginDate(beginDate);
			dd.setEndDate(curDate);
			Map<String, Object> customerOrderRate = dynamicDao.getTwoTwoOneGMVRangeForWeek(dd,cityNO,provinceNO);
			List<Map<String,Object>> lst_data = (List<Map<String, Object>>) customerOrderRate.get("lst_data");
			SimpleDateFormat sdf  = new SimpleDateFormat("yyyy-MM-dd");
			String maxDateStr = curDate;    
		    String minDateStr = "";    
	...
	public Map<String, Object> getUserBehaviorByLog(DynamicDto dynamicDto,PageInfo pageInfo) {
		...
		StoreManager storeManager = (StoreManager)SpringHelper.getBean("storeManager");
		DynamicDao dynamicDao = (DynamicDao)SpringHelper.getBean(DynamicDao.class.getName());
		dynamicDto.setBeginDate(DateUtils.dateFormat(new Date(dynamicDto.getBeginDate()),"yyyy-MM-dd"));
		String endDate = DateUtils.getNextDate(dynamicDto.getEndDate(),"yyyy/MM/dd");
		dynamicDto.setEndDate(DateUtils.dateFormat(new Date(endDate),"yyyy-MM-dd"));
		try {
			String cityNo = "";
			if(dynamicDto.getTarget()==0){//总部
				if(dynamicDto.getStoreId()==null||"".equals(dynamicDto.getStoreId())){//查询所有城市的门店
					...
					Store  store = (Store)storeManager.getObject(dynamicDto.getStoreId());
					dynamicDto.setStoreNumer(store.getNumber()==null?"-10000":String.valueOf(store.getNumber()));
					dynamicDto.setStoreNo(String.valueOf(store.getStoreno()==null?"-10000":store.getPlatformid()));
				}
			}else if(dynamicDto.getTarget()==1){//城市总监
				if(dynamicDto.getStoreId()==null||"".equals(dynamicDto.getStoreId())){//查询所有城市的门店
					if(dynamicDto.getCityId()==-10000){
				...
				Store store = (Store)storeManager.getObject(dynamicDto.getStoreId());
				dynamicDto.setStoreNo(String.valueOf(store.getPlatformid()));
			}
			if(dynamicDto.getSearchstr().equals("user_active_store")){
				result= dynamicDao.getUserBehaviorByLog(dynamicDto,cityNo,pageInfo);
			}else if(dynamicDto.getSearchstr().equals("user_active_city")){
				result= dynamicDao.getUserBehaviorByLog(dynamicDto,cityNo,pageInfo);
			}else if(dynamicDto.getSearchstr().equals("product")){
				result= dynamicDao.getStoreMember(dynamicDto,cityNo, pageInfo);
			}else if(dynamicDto.getSearchstr().equals("seach_behive")){
	...
	public Map<String, Object> exportUserBehaviorByLog(DynamicDto dynamicDto) {
		Map<String,Object> result  = new HashMap<String,Object>();
		Map<String,Object> map  = this.getUserBehaviorByLog(dynamicDto, null);
		if("success".equals(map.get("status"))){//成功返回数据
			List<Map<String, Object>> list = (List<Map<String, Object>>)map.get("user_active");
			if(list==null||list.size()==0){
				result.put("message","没有符合条件的数据！");
				result.put("status","null");
				return result;
			...
			if(dynamicDto.getSearchstr().equals("user_active_city")){
				...
				row = sheet.createRow(0);
				str_headers = new String[]{"城市","门店名称","门店编号","新增社员数","199新增开卡数","累计社员总数"};
				headers_key = new String[]{"city_name","name","storeno","nowcount","count199","opencount"};
			}else if(dynamicDto.getSearchstr().equals("try_member")){
				sheet = wb.createSheet("门店试用会员");
				row = sheet.createRow(0);
				str_headers = new String[]{"城市","门店名称","门店编号","新增试用社员数（含过期）","累计试用社员总数（有效期内）"};
				headers_key = new String[]{"city_name","name","storeno","nowcount","opencount"};
			}
			for(int i = 0;i < str_headers.length;i++){
				HSSFCell cell = row.createCell(i);
				cell.setCellStyle(getHeaderStyle());
			...
			file_xls = new File(str_file_dir_path + File.separator+System.currentTimeMillis()+"_user_behave.xls");
			if(file_xls.exists()){
				file_xls.delete();
			}
			FileOutputStream os = null;
