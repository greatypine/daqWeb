...
public class PlatformStoreDaoImpl extends DAORootHibernate implements PlatformStoreDao {
	...
	public List<PlatformStore> getPlatformStoreList(String where, PageInfo pageInfo) {
		...
		try {
			...
			lst_data = query.setFirstResult(pageInfo.getRecordsPerPage() * (pageInfo.getCurrentPage() - 1))
		...
		/*
		 * if(lst_data == null){ lst_data = new ArrayList<Map<String,Object>>(); }
		 * map_result.put("pageinfo", pageInfo); map_result.put("header", "");
		 ...
		 * query.addEntity(StoreDTO.class); List<StoreDTO> list = query.list();
		 * pageInfo.setTotalRecords(list.size()); List<StoreDTO> returnList =
		 * query.setFirstResult( pageInfo.getRecordsPerPage() *
		 * (pageInfo.getCurrentPage() - 1))
		 * .setMaxResults(pageInfo.getRecordsPerPage()).list(); return returnList;
		 */
	}
	...
	public List<Map<String, Object>> getEmployeeByStore(String storeNo, Boolean isRealtime) {
		...
		String str_sql = "select concat(te.id,'') as employeeId,CONCAT(te.store_id,'') as platformid,te.name as employeeName,ts.name as storeName,te.code as employeeNo,te.online as online from t_employee te INNER JOIN t_store ts ON ts.id = te.store_id where te.status = 0 "
				+ sql;
		Session session = getHibernateTemplate().getSessionFactory().openSession();
	...
	public List<Map<String, Object>> getEmployeeByEmployeeNo(String employeeNo, Boolean isRealtime) {
		...
		String str_sql = "select concat(te.id,'') as employeeId,te.name as employeeName,ts.name as storeName,te.code as employeeNo,te.`online` as online from t_employee te INNER JOIN t_store ts ON ts.id = te.store_id where te.status = 0"
				+ sql;
		Session session = getHibernateTemplate().getSessionFactory().openSession();
		List<Map<String, Object>> lst_result = new ArrayList<Map<String, Object>>();
		try {
	...
	public List<Map<String, Object>> getEmployeeByCity(String city_code, Boolean isRealtime) {
		...
		String sql = "select concat(te.id,'') as employeeId,CONCAT(te.store_id,'') as platformid,te.name as employeeName,ts.name as storeName,te.code as employeeNo,te.online as online,te.mobilephone as mobilephone from t_employee te "
				+ "INNER JOIN t_store ts ON ts.id = te.store_id where  te.status = 0 and ts.name NOT LIKE '%测试%' and ts.white!='QA'"
				+ str_sql;
		Session session = getHibernateTemplate().getSessionFactory().openSession();
		List<Map<String, Object>> lst_result = new ArrayList<Map<String, Object>>();
		try {
	...
	public List<Map<String, Object>> getGoodsTypeCount(String dd) {
		// TODO Auto-generated method stub
		/**
		 * @author wuxinxin 查询新老用户增长量 按天查10天 2018年5月17日
		 */
	...
	public List<Map<String, Object>> getRetrenchMoney(String dd) {
		 ...
		 */
		String retrenchMoneySql = "select tc.mobilephone,ts.name,tmi.sum_retrench_money from t_mebmer_info tmi,t_customer tc,t_store ts where tmi.customer_id=tc.id and tc.storeId=ts.id order by tmi.sum_retrench_money desc limit 10";
		Session session = getHibernateTemplate().getSessionFactory().openSession();
		try {
	...
	public List<Map<String, Object>> getHaveRebate(String dd) {
		 ...
		 * @author wuxinxin 2018年6月6日
		 */
		String haveRebateSql = "select tc.mobilephone,ts.name,tmi.remain_rebate from t_mebmer_info tmi,t_customer tc,t_store ts where tmi.customer_id=tc.id and tc.storeId=ts.id order by tmi.remain_rebate desc limit 10";
		Session session = getHibernateTemplate().getSessionFactory().openSession();
		try {
	...
	public List<Map<String, Object>> getUsedRebate(String dd) {
		 ...
		 * @author wuxinxin 2018年6月6日
		 */
		String usedRebateSql = "select tc.mobilephone,ts.name,sum(tmi.sum_rebate)-sum(tmi.remain_rebate) from t_mebmer_info tmi,t_customer tc,t_store ts where tmi.customer_id=tc.id and tc.storeId=ts.id order by sum(tmi.sum_rebate)-sum(tmi.remain_rebate) desc limit 10";
		Session session = getHibernateTemplate().getSessionFactory().openSession();
		try {
	...
	public List<Map<String, Object>> getSumRebate(String dd) {
		 ...
		 * @author wuxinxin 2018年6月6日
		 */
		String sumRebateSql = "select tc.mobilephone,ts.name,tmi.sum_rebate from t_mebmer_info tmi,t_customer tc,t_store ts where tmi.customer_id=tc.id and tc.storeId=ts.id order by tmi.sum_rebate desc limit 10";
		Session session = getHibernateTemplate().getSessionFactory().openSession();
		try {
	...
	public List<Map<String, Object>> getAllRetrench(String dd) {
		...
		String sumEetrenchSql = "select sum(tmi.sum_retrench_money)  sumall,sum(tmi.sum_retrench_money)-(sum(tmi.sum_rebate)-sum(tmi.remain_rebate)) subprice,sum(tmi.sum_rebate)-sum(tmi.remain_rebate) usedrebate from t_mebmer_info tmi";
		Session session = getHibernateTemplate().getSessionFactory().openSession();
		try {
			SQLQuery sqlQuery = session.createSQLQuery(sumEetrenchSql);
			// 获得查询数据
			List<Map<String, Object>> turnover_data = sqlQuery.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			return turnover_data;
		} catch (Exception e) {
