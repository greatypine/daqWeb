...
//		List<Map<String,Object>> lst_result = new ArrayList<Map<String,Object>>();
	...
	public Map<String, Object> queryMemberDataList(MemberDataDto memberDataDto, PageInfo pageInfo){
		...
		if(StringUtils.isNotEmpty(memberDataDto.getMemberType())){
			sql = sql + " and (";
			String[] names = memberDataDto.getMemberType().split(",");
			for (int i = 0; i < names.length; i++) {
				if ("199付费社员".equals(names[i].trim())) {
					sql = sql + " dum.member_type in ('associator_start_2','yearCard','yearCard-19') ";
				}else if ("安心存".equals(names[i].trim())){
					sql = sql + " dum.associator_mode = 'axc' ";
				}else if ("畅卡升级".equals(names[i].trim())){
					sql = sql + " dum.associator_mode = 'oneToTwo-19' ";
    ...
    public Map<String, Object> queryRemainMemberList(MemberDataDto memberDataDto, PageInfo pageInfo) {
        ...
        String sql="select dum.customer_id,CASE dum.customer_source WHEN 'app' THEN 'APP' WHEN 'callcenter' THEN '400客服' WHEN 'store' THEN '门店' WHEN 'wechat' THEN '微信' WHEN 'pad' THEN '智能终端' WHEN 'web' THEN 'WEB' WHEN 'citic' THEN '中信用户联盟' WHEN 'tv' THEN '电视' WHEN 'third_party' THEN '第三方' WHEN 'action' THEN '活动'ELSE '无' END AS customer_source," +
                "dum.mobilephone,strleft(dum.regist_time,19) as regist_time,strleft(dum.opencard_time,19) as opencard_time,IFNULL(dum.inviteCode,'') as invitecode,ifnull(tmi.sum_rebate,0) as sum_rebate,ifnull(tmi.remain_rebate,0) as remain_rebate," +
